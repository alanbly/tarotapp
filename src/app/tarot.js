...
export const Cards = Object.freeze({
  FOOL: Object.freeze(new Card('TheFool.png', Suits.MAJOR, Ranks.KEY, Facings.LEFT, 0, 57, 'The Fool',
    'The Spirit of Æther (&Alpha;&iota;&theta;&eta;&rho;)',
    "",
  ...
  THREE_OF_CUPS: Object.freeze(new Card('ThreeOfCups.png', Suits.CUPS, Ranks.THREE, Facings.NONE, 2, 55, 'The Three of Cups',
    ...
    "Abundance, plenty, success, pleasure, sensuality, passive success, good luck and fortune; love, gladness, kindness, liberality.\nBinah of <span direction='rtl'>&#x05D4</span> (Plenty, hospitality, eating and drinking, pleasure, dancing, new clothes, merriment).")),
...
export class Spread {
  ...
  getCardPlacement(idx, facing) {
    const placements = this.placements[facing] || this.placements[Facings.LEFT];
    return placements[idx];
  }

  getCardPosition(width, height, idx, facing) {
    return this.positionFromPlacement(width, height, this.getCardPlacement(idx, facing));
  }
  ...
  getStyles(/*styles*/ {anchor, card, tooltip}, width, height, offset, insetX = 0, insetY = 0, facing = Facings.LEFT, ) {
    ...
    return placements.map((placement, idx) => {
      ...
      const anchorCls = `${anchor}:nth-child(${idx + offset + 1})`;
      return `
        .${anchorCls} {
          left: ${x}px;
          top: ${y}px;
  ...
  CELTIC: Object.freeze(new Spread('Celtic Divination', 
    new Placement(1, 1, Rotations.UPRIGHT, `Deck`), 
    {
      [Facings.RIGHT]: [
        ...
        new Placement(2.05, 4.05, Rotations.UPRIGHT, `This covers the Significator. This card gives the influence which is affecting the Querent or Question generally, the atmosphere of it in which the other currents work.`),
        ...
        new Placement(2, 1, Rotations.UPRIGHT, `This crowns the Significator. It represents (a) the Querent's aim or ideal in the Question; (b) the best that can be achieved under the circumstances, but that which has not yet been made actual.`),
        new Placement(2, 7, Rotations.UPRIGHT, `This is beneath the Significator. It shows the foundation or basis of the Question, that which has already passed into actuality and which the Significator has made his own.`),
